...
public class ServerSender {
    ...
    public static void sendDefeatedBossDataToAllPlayers(ServerLevel level, String bossId, String killer, boolean defeated,
        ...
        for (ServerPlayer player : level.getServer().getPlayerList().getPlayers()) {
            ...
            buf.writeUtf(bossId);
            buf.writeUtf(killer);
            buf.writeBoolean(defeated);
            buf.writeBoolean(true);
            buf.writeUtf(startTime);
            buf.writeUtf(endTime);
            buf.writeUtf(attemptTop3);
            buf.writeUtf(globalTop3);

            NetworkManager.sendToPlayer(player, new ResourceLocation("boss_checklist", "boss_defeated"), buf);
        }
    }
