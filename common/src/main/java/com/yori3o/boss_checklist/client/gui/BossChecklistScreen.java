...
public class BossChecklistScreen extends Screen {
    ...
    /*private static final ResourceLocation bar_background = // FOR 1.20.1
        new ResourceLocation("boss_checklist", "textures/gui/white_background.png");
    private static final ResourceLocation bar_progress = 
        new ResourceLocation("boss_checklist", "textures/gui/white_progress.png");*/

    private static final ResourceLocation BOOKMARK_SETTINGS =
        ResourceLocation.fromNamespaceAndPath("boss_checklist", "textures/gui/bookmark_settings.png");
    ...
    public void init() {
        ...
        if ((!bossesListAreLoaded) || exitFromSettings) {
            ...
            for (BossData b : bossDataList) {
                if (BossChecklistClient.isProgressionMode_dynamic && !ClientDataSaver.defeatedBossesIds_InWorld.contains(b.getId())) {
                    ...
                    String translated = Component.translatable("boss_checklist.boss." + b.getId().replace(":", "_")).getString();
                    // truncate
    ...
    private void updatePage() {
        ...
        for (int i = fromIndex; i < toIndex; i++) {
            ...
            CustomCheckbox cb = new CustomCheckbox(x, y, Component.literal(boss), DATA_SAVER.isBossDefeated(bossId), true,
                checked -> {ClientDataSaver.BossDefeated(bossId, checked); updateCounts();},
                () -> Minecraft.getInstance().setScreen(new BossInfoScreen(new BossChecklistScreen(false), bossId))
            );
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.blit(BOOKMARK_SETTINGS, bookX + BOOKMARK_SETTINGS_x, bookY + BOOKMARK_SETTINGS_y, 0, 0, 12, 22, 12, 22);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (mouseX >= ((this.width - 512) / 2) + BOOKMARK_SETTINGS_x  && mouseX <= ((this.width - 512) / 2) + BOOKMARK_SETTINGS_x + 12 && mouseY >= ((this.height - 256) / 2) + BOOKMARK_SETTINGS_y && mouseY <= ((this.height - 256) / 2) + BOOKMARK_SETTINGS_y + 22) {
            minecraft.setScreen(new ConfigScreen(new BossChecklistScreen(true)));
        }
